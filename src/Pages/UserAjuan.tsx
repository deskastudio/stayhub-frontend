...
const UserAjuan: React.FC = () => {
  ...
  const handleFormSubmit = (data: Omit<Ajuan, "id">) => {
    const currentAjuanList = JSON.parse(localStorage.getItem("ajuanList") || "[]");
    const newAjuan = { ...data, id: currentAjuanList.length + 1 }; // Generate id baru
    localStorage.setItem("ajuanList", JSON.stringify([...currentAjuanList, newAjuan]));
  ...
  return (
    <div className="p-8 flex-grow">
      ...
      <AjuanModal isOpen={isModalOpen} onClose={closeModal} onSubmit={handleFormSubmit} />

      {/* Proses Pengajuan Keluhan */}
      <div className="flex justify-center items-center py-10">
        <div className="bg-blue-50 p-8 rounded-lg shadow-md w-full max-w-4xl">
          <h2 className="text-center text-3xl font-bold mb-20">Proses Pengajuan Keluhan</h2>
