...
const FilterButton: React.FC<FilterButtonProps> = ({ options, onFilter }) => {
  ...
  return (
    <div className="relative">
      <button className="bg-white text-primary p-2 rounded-lg flex items-center space-x-2" onClick={() => setIsOpen(!isOpen)}>
      ...
      {isOpen && (
        <ul className="absolute bg-white shadow-lg rounded-lg mt-2 w-40 z-10">
          {options.map((option, index) => (
            <li key={index} onClick={() => handleFilterChange(option)} className="p-2 hover:bg-primary hover:text-white cursor-pointer rounded">
              {option}
            </li>
          ))}
        </ul>
